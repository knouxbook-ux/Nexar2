...
export class SubscriptionService extends EventEmitter {
  ...
  async activateLicenseKey(key: string, userId: string): Promise<Subscription> {
    Logger.info('Activating license key via backend...', { key });

    // Call the real backend endpoint to validate and activate the key
    const baseUrl = typeof process !== 'undefined' && process.env
      ? (process.env.EXPO_PUBLIC_API_URL ?? '')
      : '';

    const apiUrl = baseUrl
      ? `${baseUrl}/api/trpc/licenses.activate`
      : '/api/trpc/licenses.activate';
    ...
    if (!response.ok) {
      ...
      let errMsg = `License activation failed (${response.status})`;
      try {
    ...
    const activated = result?.result?.data?.json ?? result;

    // Map backend response to our Subscription type
    const subscription: Subscription = {
      userId,
      tier: (activated?.plan ?? 'pro') as 'free' | 'pro' | 'premium' | 'enterprise',
      plan: SUBSCRIPTION_PLANS[activated?.plan as keyof typeof SUBSCRIPTION_PLANS] ?? SUBSCRIPTION_PLANS.pro,
      status: 'active',
  ...
  async applyReferralCode(code: string): Promise<number> {
    Logger.info('Applying referral code via backend', { code });
    try {
      const baseUrl = process.env.EXPO_PUBLIC_API_URL ?? "http://localhost:3000";
      const res = await fetch(`${baseUrl}/api/trpc/referrals.validate`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
      ...
      return data?.result?.data?.json?.discount ?? 0;
    } catch (err) {
      Logger.error('Failed to validate referral code', err);
      return 0;
    }
  }
  ...
  private async processPayment(details: {
    ...
    Logger.info('Processing payment via backend tRPC...', { planId: details.planId });

    try {
      // Delegate to backend payments.create endpoint which handles Stripe/PayPal
      const baseUrl = process.env.EXPO_PUBLIC_API_URL ?? "http://localhost:3000";
      const res = await fetch(`${baseUrl}/api/trpc/payments.create`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({
          json: {
            ...
            description: `Subscription plan: ${details.planId}`,
            provider: details.method.provider ?? "stripe",
          },
        }),
      });

      const data = await res.json();
      const backendPayment = data?.result?.data?.json;

      const payment: PaymentInfo = {
        id: backendPayment?.id ?? `pay_${Date.now()}`,
        userId: details.userId,
        amount: details.amount,
        currency: details.currency,
        method: details.method,
        status: backendPayment?.status === "completed" ? "completed" : "pending",
        createdAt: new Date(),
        completedAt: backendPayment?.status === "completed" ? new Date() : undefined,
        metadata: { planId: details.planId, backendId: backendPayment?.id },
      };

      this.payments.push(payment);
      this.emit('paymentProcessed', payment);
      Logger.info('Payment processed successfully via backend', { id: payment.id });
      return payment;
  ...
  getRemainingAICredits(): number {
    // Reads from the subscription loaded via loadSubscription() which is persisted to SecureStore.
    // The subscription itself is set by the backend (tRPC subscriptions.current) on login.
...
// ─── Named instance export with method aliases ─────────────────────────────
const _instance = SubscriptionService.getInstance();

// Wrap instance with commonly needed aliases
export const subscriptionService = _instance;
export { SubscriptionService };

// Augment prototype with alias methods expected by screens
if (!SubscriptionService.prototype.getSubscriptionPlans) {
  SubscriptionService.prototype.getSubscriptionPlans = function() {
    return this.getAllPlans();
  };
}

if (!SubscriptionService.prototype.getPlans) {
  SubscriptionService.prototype.getPlans = function() {
    return this.getAllPlans();
  };
}

if (!SubscriptionService.prototype.purchaseSubscription) {
  SubscriptionService.prototype.purchaseSubscription = async function(userId: string, planId: string, paymentMethod: string) {
    const tier = planId.replace('plan_', '') as any;
    return this.subscribe(tier, paymentMethod as any);
  };
}

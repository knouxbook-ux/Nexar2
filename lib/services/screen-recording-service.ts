...
export class ScreenRecordingService extends EventEmitter {
  ...
  private async startNativeRecording(): Promise<void> {
    // Native screen recording via Android MediaProjection API
    // Requires custom native module (NexarScreenCapture) linked via npx expo prebuild
    Logger.debug('Starting native screen recording via MediaProjection');
    try {
      const { NativeModules } = require('react-native');
      if (NativeModules.NexarScreenCapture) {
        await NativeModules.NexarScreenCapture.startRecording({
          outputPath: this.currentRecordingPath,
          audioSource: this.currentConfig?.audioSource ?? 'both',
        });
        return; // Native module handles the rest
      }
      Logger.debug('NexarScreenCapture not linked — run: npx expo prebuild');
    } catch (err) {
      Logger.debug('Native module error:', err);
    }
  ...
  private async stopNativeRecording(): Promise<void> {
    Logger.debug('Stopping native screen recording');
    try {
      const { NativeModules } = require('react-native');
      if (NativeModules.NexarScreenCapture) {
        await NativeModules.NexarScreenCapture.stopRecording();
        return;
      }
    } catch (err) {
      Logger.debug('Native stop error (module not linked):', err);
    }
    // Ensure path exists for downstream FileSystem.getInfoAsync calls
    const exists = await FileSystem.getInfoAsync(this.currentRecordingPath);
    if (!exists.exists) {
      await FileSystem.makeDirectoryAsync(
        this.currentRecordingPath.substring(0, this.currentRecordingPath.lastIndexOf('/')),
        { intermediates: true }
      ).catch(() => {});
    }
  }

...
import {
  ...
  Animated, ActivityIndicator, SafeAreaView, Alert,
} from 'react-native';
...
const SUPPORT_RESPONSES: Record<string, string> = {
  'تسجيل': '📹 يدعم Nexar Pro التسجيل بدقة تصل إلى 8K بمعدل 120fps. تحتاج إلى اشتراك Pro أو Premium للوصول لأعلى الدقات.',
  'اشتراك': '💳 تتوفر ثلاثة خطط:\n• مجاني: 720p، 30 دقيقة\n• Pro (9.99$/شهر): 1080p، غير محدود\n• Premium (19.99$/شهر): 4K/8K + كل المميزات',
  'مشكلة': '🔧 شكراً لإبلاغك! يرجى وصف المشكلة بالتفصيل وسيتواصل معك فريقنا خلال 24 ساعة على support@nexarpro.app',
  'ترخيص': '🔑 يمكنك إدارة مفاتيح الترخيص من: الرئيسية ← License Keys. إذا واجهت مشكلة، يرجى مشاركة مفتاحك معنا.',
      ...
      <View style={[styles.messageWrapper, isUser ? styles.userWrapper : styles.otherWrapper]}>
        ...
        <View style={[styles.bubble, isUser ? styles.userBubble : styles.otherBubble]}>
          ...
          <View style={styles.msgMeta}>
            ...
            {isUser && (
              <Text style={{ color: item.status === 'delivered' ? '#A78BFA' : '#6b7280', fontSize: 11, marginLeft: 4 }}>
        ...
        <Animated.View style={{ flex: 1, opacity: fadeAnim }}>
          ...
          <View style={styles.header}>
            ...
            <View style={{ flex: 1, alignItems: 'center' }}>
              ...
              <View style={styles.onlineIndicator}>
                ...
                <Text style={styles.onlineText}>متصل الآن · عادةً يرد خلال دقائق</Text>
              </View>
            </View>
            <TouchableOpacity
              onPress={() => Alert.alert('📧 تواصل بالبريد', 'support@nexarpro.app')}
          ...
          >
              ...
              ListFooterComponent={
                isTyping ? (
                  <View style={[styles.otherWrapper, { marginBottom: 8 }]}>
                    ...
                    <View style={[styles.bubble, styles.otherBubble, { paddingVertical: 12, paddingHorizontal: 16 }]}>
                      <View style={{ flexDirection: 'row', gap: 5, alignItems: 'center' }}>
                        {[0, 1, 2].map((i) => (
                          <Animated.View key={i} style={styles.typingDot} />
                        ))}
                        <Text style={{ color: '#9CA3AF', fontSize: 12, marginLeft: 4 }}>يكتب...</Text>
                      </View>
                    </View>
                  </View>
                ) : null
            ...
            {messages.length <= 2 && (
              <View>
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <TouchableOpacity onPress={() => sendMessage(item)} style={styles.quickReply}>
            ...
            <BlurView intensity={60} tint="dark" style={styles.inputBar}>
              ...
              >
                <LinearGradient
                  colors={inputText.trim() ? ['#7C3AED', '#EC4899'] : ['#27272a', '#27272a']}
...
const styles = StyleSheet.create({
  ...
  onlineIndicator: { flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 3 },
  ...
  senderName: { color: '#A78BFA', fontSize: 11, fontWeight: '700', marginBottom: 2 },

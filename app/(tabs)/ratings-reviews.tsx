...
export default function RatingsReviewsScreen() {
  ...
  const {
    ...
    refetch,
  } = trpc.ratings.list.useQuery({ limit: 30 });
  ...
  const handleSubmit = () => {
    if (!user) {
      Alert.alert(
        ...
        ar
          ? "يرجى تسجيل الدخول لإرسال تقييم"
    ...
    if (selectedRating === 0) {
      Alert.alert(
        ar ? "خطأ" : "Error",
        ar ? "يرجى اختيار تقييم من 1-5" : "Please select a rating (1-5)",
      );
      return;
    }
  ...
  const renderStars = (count: number, size = 18, filled = true) =>
    [1, 2, 3, 4, 5].map((i) => (
      <Text
        ...
        style={{
          fontSize: size,
          color: i <= count ? "#A78BFA" : "rgba(255,255,255,0.15)",
        }}
      >
        ★
      </Text>
        ...
        {loadingReviews ? (
          ...
          reviews.map((review: any) => (
            ...
            >
              ...
              {review.comment && (
                <Text
                  style={{ color: "#D1D5DB", fontSize: 13, lineHeight: 20 }}
                >
                  {review.comment}
                </Text>
              )}

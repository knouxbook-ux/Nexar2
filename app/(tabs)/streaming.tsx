...
const PLATFORMS: {
  ...
  },
  {
    ...
    icon: "💙",
    color: "#1877F2",
    rtmpBase: "rtmps://live-api-s.facebook.com/rtmp/",
  },
];
...
export default function StreamingScreen() {
  ...
  const handleGo = async () => {
    ...
    if (!streamKey.trim()) {
      Alert.alert(
        "Stream Key Required",
        "Enter your stream key from the platform dashboard.",
        [{ text: "OK" }],
      );
      return;
    }
    ...
    try {
      ...
      await StreamingService.initialize({
        ...
        frameRate: 60,
        videoBitrate:
          quality === "1080p" ? 6000 : quality === "720p" ? 4500 : 3000,
        audioBitrate: 128,
          ...
          >
            ...
            >
              <View style={{ flex: 1 }}>
                ...
                <Text style={s.headerSub}>
                  RTMP · YouTube · Twitch · TikTok · Facebook
                </Text>
              </View>
              <Animated.View
                style={{
                  transform: [
                    { scale: isLive ? livePulse : new Animated.Value(1) },
                  ],
                }}
              >
                <StatusPill
                ...
                <View style={s.metricsGrid}>
                  {[
                    ...
                    },
                    {
                      icon: "👁",
                      label: "Viewers",
                      value: metrics.viewerCount.toLocaleString(),
                    },
                    { icon: "🎞", label: "FPS", value: `${metrics.fps}` },
                    {
                      icon: "📶",
                      label: "Bitrate",
                      value: `${(metrics.uploadSpeed / 1000).toFixed(1)} Mbps`,
                    },
                  ].map((item) => (
                ...
                {PLATFORMS.map((plat) => (
                  <TouchableOpacity
                    ...
                    style={[
                      s.platformBtn,
                      platform === plat.id && {
                        borderColor: plat.color + "80",
                      },
                    ]}
                  >
                    {platform === plat.id && (
                      <LinearGradient
                        colors={[plat.color + "25", plat.color + "0a"]}
                        style={StyleSheet.absoluteFill}
                      />
                    )}
                    ...
                    <Text
                      style={[
                        s.platformLabel,
                        platform === plat.id && { color: plat.color },
                      ]}
                    >
                      {plat.label}
                    </Text>
            ...
            <View style={s.glassCard}>
              ...
              <Text style={s.settingDesc}>
                Get from {activePlatform.label} Studio → Live → Stream Settings
              </Text>
              <TextInput
                value={streamKey}
              ...
              <View style={{ flexDirection: "row", gap: 10, flexWrap: "wrap" }}>
                {QUALITIES.map((q) => (
                  ...
                  >
                    ...
                    <Text
                      style={[s.qualityTxt, quality === q && { color: "#fff" }]}
                    >
                      {q}
                    </Text>
                  </TouchableOpacity>
              ...
              <LinearGradient
                colors={
                  isLive
                    ...
                    : isConnecting
                      ? ["#4b5563", "#374151"]
                      : [activePlatform.color, activePlatform.color + "aa"]
                }
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 0 }}
                style={s.goBtn}
              >
                <Text style={s.goBtnText}>
                  {isLive
                    ...
                    : isConnecting
                      ? "⏳  Connecting…"
                      : `🔴  Go Live on ${activePlatform.label}`}
                </Text>
              </LinearGradient>
            </TouchableOpacity>
            ...
            {isLive && (
              <TouchableOpacity
                onPress={() => StreamingService.pauseStream()}
                activeOpacity={0.85}
                style={s.pauseBtnWrap}
              >
